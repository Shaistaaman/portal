...
| **Calendar — view** (see §4.5)                                                            |                                                    ✅ all properties                                                     |                                             🟡 own properties + own created bookings (all properties)                                             |                  ✅ all properties                   |                         ❌                         |
| **Calendar — create booking**                                                             |                                              ✅ Self / Guest / Maintenance                                               |                                                       🟡 Self / Maintenance, own properties                                                       |                   🟡 Self / Guest                    |                         ❌                         |
| **Calendar — edit/delete booking**                                                        |                                             ✅ any booking, all transitions                                              |                                                 🟡 own-created bookings; status → Completed only                                                  |   🟡 own-created bookings; status → Completed only   |                         ❌                         |
...
- **Admin:** sees all properties' calendars; can create, view, and edit any booking.
- **Owner:** sees own properties' calendars PLUS all bookings they created (even on properties they don't own). Can create Self (on all active properties) and Maintenance (on own properties only). Can edit only own-created bookings.
- **Agent:** sees all properties' calendars (view-only for others' bookings). Can create Self and Guest bookings on all active properties. Can edit only own-created bookings. Mark Confirm action available only on own Payment Pending bookings after checkout.
...
- Confirmed → Completed — **admin / owner / agent**, **only once checkout date < today** (owner: Mark Complete button on dashboard only for own-created confirmed bookings; agent: Mark Confirm button on dashboard only for own-created confirmed bookings)
...
The status dropdown is gated so owner/agent effectively only ever see **Completed** (when a Confirmed/Payment Pending booking's checkout has passed); everything else is read-only to them.
...
- **WhatsApp support button added** — a green floatable button (bottom-right, fixed position, visible on all admin pages) and a menu option in the profile dropdown (top-right header) both open WhatsApp with a pre-filled message linking to support: +39 366 170 7510 and +39 331 799 5308. Built using `utils/whatsapp.ts` and the `WhatsAppFloatingButton` component, wired into `AdminLayout` and `ProfileMenu`.
- **Notifications page built** — admin-only feature at `/admin/notifications`. Shows a paginated list of system events (property added/approved/rejected, booking created/confirmed/completed/payment received, user registered, financial uploaded, package created). Each notification has a read/unread state (blue dot for unread), action-type badge (color-coded), timestamp (relative: "5m ago" / "2d ago"), and click-to-navigate to the relevant section. Unread-only toggle + Mark All Read button. The header's "View all Notifications" button navigates here and closes the popup.
- **Dashboard KPI cards expanded** — added Portfolio Health (4 cards: Active, In Review, Inactive, Rejected property counts) and Booking Health (3 cards: Bookings This Month, Completion Rate %, No-Show Rate %) below the existing top 6 cards. All metrics computed live from mock data (`MOCK_PROPERTIES` and `MOCK_BOOKINGS`), color-coded by health (green for good, red for risk, neutral/blue for informational).
- **Profile dropdown simplified** — removed "My Profile", "Change Password", and "Help & Support" options. Now contains only "Settings" (links to `/admin/settings`) and "Contact Support (WhatsApp)".
- **Language selector removed entirely** — deleted `components/layout/LanguageSelector.tsx` and removed all references from `AdminLayout.tsx` and `PublicHeader.tsx`. Portal and marketing site now default to English only (no language toggle).
- **Book Your Stay button removed** — removed from `PublicHeader.tsx` desktop and mobile sections; marketing site header now shows nav links only, no CTA button.
- **Owner dashboard enhanced** — dashboard now displays an "All Bookings" table below KPIs, showing all bookings for owner's properties. Features: 6 items per page with pagination, search by property/guest name, status filter dropdown (All/Confirmed/Payment Pending/Completed/Cancelled), "View on Calendar" button (filters calendar to property), and "Mark Complete" action for non-completed bookings. Bookings table styled as User-Management-style table with property name, guest name/phone, check-in/out dates, and status badges.
- **Owner financial records scoped** — created `OwnerFinancialPage` that filters to show only records tagged to the owner's ID or untagged records. The "Tag To" filter is hidden entirely for owner view (admin-only feature). Maintains Category and Year filters. Same pagination (8 items per page) and Download/Delete actions as admin version.
- **Property Calendar filtering implemented** — clicking "Property Calendar" from PropertyList's 3-dot action menu now navigates to calendar with `?property=<id>` query param. Calendar component accepts `initialPropertyFilter` prop, pre-filtering to show only that property's bookings. Works for both admin and owner roles. Routes updated: `/admin/calendar?property=<id>`, `/owner/calendar?property=<id>`, `/agent/calendar?property=<id>`. Also available from Owner Dashboard bookings table "View on Calendar" button.
- **Notifications page built for all roles** — created shared `NotificationsPage` component used by admin, owner, agent, and client. Displays paginated system event list (read/unread state, action-type badge, timestamp, click-to-navigate). Features: unread-only toggle, "Mark All Read" button, search by event title. Routes: `/admin/notifications`, `/owner/notifications`, `/agent/notifications`, `/client/notifications`. Accessible via header's "View all Notifications" button (closes popup, navigates to full page).
- **Agent profile verification flow introduced** — agent login flow: if profile is incomplete (checked via `DEMO_AGENT_PROFILE_INCOMPLETE` flag for now; will use auth context in production), dashboard shows red alert banner: "Unlock the SkyLife full potential, please complete your profile" with "Go to Profile" button. All left menu items disabled/grayed-out (Dashboard, Calendar, Financial) except Settings (enabled for profile completion) and SignOut. Production logic: check if specific agent fields are empty in auth context. After "I Confirm" button in Settings, menu items enabled and user redirected to Dashboard. Persistence: resets on page refresh (demo mode).
- **Agent layout created** — `AgentLayout` mirrors `OwnerLayout` structure: collapsible sidebar (Dashboard, Calendar, Financial, Settings, SignOut), header with notifications bell + profile dropdown (Settings + WhatsApp), WhatsApp floating button, mobile menu. Left menu items show as disabled/grayed (pointer-events: none, opacity-50) until profile completion confirmed.
- **Agent Settings page built** — displays agent profile form with fields from Admin Add User page: Photo (upload), First Name, Last Name, Email, Phone, Agency Name, Agency Address, Website. "I Confirm" button at bottom marks profile complete, enables menu items, and redirects to `/agent/dashboard`. Form fields prefilled with current agent data for review on first login.
- **Agent dashboard created** — similar to Owner dashboard: 4 KPI cards (Total Properties, Active, Inactive, Bookings This Month) + bookings table below. Bookings filtered to show only `createdByRole === "agent"` (agent's own bookings). Same pagination (6 items/page), search, status filter, "View on Calendar" and "Mark Complete" actions as owner dashboard.
- **Agent financial page created** — displays records tagged to agent's ID only (`tagUserId === DEMO_AGENT_ID`). "Tag To" filter hidden. Category and Year filters available. Same table layout and pagination (8 items/page) as owner version. Routes: `/agent/financial`.
- **Dashboard bookings table action standardized** — "View on Calendar" button now available in both Owner and Agent dashboards, navigating to calendar with property filter applied (`?property=<id>`).
- **Owner & Agent dashboard "Booked By" column added** — Owner dashboard now displays 7-column table: Property, Guest, **Booked By** (badge: "Self" blue or "Other" gray), Check-in, Check-out, Status, Actions. Shows whether booking was created by owner ("Self") or by admin/agent ("Other"). Only visible on Owner dashboard (Agent sees only own-created bookings, so all are "Self").
- **Owner dashboard Mark Complete action refined** — button now appears **only on** bookings where: (1) `createdByRole === "owner"` (Self bookings), (2) `status === "confirmed"` (not payment_pending), and (3) checkout date < today (booking completed). Clarified: owner can only complete their own confirmed bookings after stay ends.
- **Agent dashboard Mark Complete action added** — button labeled "Mark Complete" appears **only on** bookings where: (1) `status === "confirmed"` (not payment_pending), and (2) checkout date < today (stay ended). Agent marks confirmed bookings as complete only after guest has checked out. Transition: Confirmed → Completed (after checkout).
- **Calendar visibility clarified for Owner** — Owner sees on calendar: (a) all bookings on own properties (any role created), (b) all bookings created by owner on any property. In Month/Timeline views, owner can edit only own-created bookings (other bookings show as non-editable, cursor-not-allowed, opacity-60). Add booking page: All Properties filter shows all active properties, but Maintenance blocks can only be created on own properties; Self bookings can be created on any active property.
- **Calendar visibility clarified for Agent** — Agent sees on calendar: all properties' bookings (viewed from calendar, no edit). Agent can edit only own-created bookings (again, cursor-not-allowed for others). Add booking page: can create Self and Guest bookings on all active properties. Agent bookings shown in dashboard filtered by `createdByRole === "agent"`.
- **Check-in/Check-out date semantics confirmed** — Booking model: check-in (inclusive, first night booked) and check-out (exclusive, departure day, not a night). Calendar correctly displays: Sept 1 check-in + Sept 5 check-out = Sept 1-4 shown in green (4 nights), Sept 5 not shown. Dashboard table displays both dates as-is (computed from booking.checkIn and booking.checkOut). This is hotel industry standard and correctly implemented.
- **Rejected property added for testing** — new mock property id 13 "Portofino Harbor Villa" (owned by Giulia Romano, status: rejected, rejection reason: "Property documentation incomplete…"). Allows testing owner's resubmit flow on Properties page.
- **Owner menu icons updated** — OwnerLayout now uses lucide-react icons: LayoutDashboard (Dashboard), Building2 (Properties), Calendar (Calendar), DollarSign (Financial). Matches Admin layout icon set for consistency.
- **Admin dashboard icon updated** — Dashboard nav item now uses LayoutDashboard icon (instead of Home) in AdminLayout via admin-nav.ts, matching Owner layout and modern lucide icon set.
