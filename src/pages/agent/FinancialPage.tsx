...
/**
 * Demo-only agent id matching the mockUsers dataset (Marco Bianchi).
 */
const DEMO_AGENT_ID = "3";
...
export default function AgentFinancialPage() {
  ...
  // Filter to only records tagged to this agent
  const agentRecords = useMemo(
    () =>
      records.filter(
        (r) => r.tagUserId === DEMO_AGENT_ID || r.tagUserId === "",
      ),
    [records],
  );
  ...
  const years = useMemo(
    () =>
      Array.from(new Set(agentRecords.map((r) => r.periodYear))).sort(
        (a, b) => b - a,
      ),
    [agentRecords],
  );
  ...
  }, [agentRecords, search, categoryFilter, yearFilter]);
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-semibold text-neutral-950">Financials</h1>
